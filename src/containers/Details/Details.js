...
export default function Details() {
  ...
  const { full, name, totalIssues } = useParams();
  ...
  React.useEffect(() => {
    if (!issues["page_" + (+pageNumber + 1)]) {
      fetchIssues(full, name, pageNumber + 1).then((res) => {
        dispatch(addIssues({ full, res, pageNumber: pageNumber + 1 }));
      });
    }
  }, [pageNumber]);
  return (
    <div className="max-w-7xl mx-auto p-3">
      ...
      {issues["page_" + (+pageNumber + 1)] ? (
        <IssueList
          ...
          issues={issues["page_" + (+pageNumber + 1)]}
        />
      ) : (
